...
app_units::enable() {
  app_units::run systemctl "$@" --quiet enable || softfail --exit-status $? || return $?
}

app_units::enable_now() {
  app_units::run systemctl "$@" --quiet --now enable || softfail --exit-status $? || return $?
}

app_units::disable() {
  app_units::run systemctl "$@" --quiet disable || softfail --exit-status $? || return $?
}

app_units::disable_if_exists() {
  app_units::run_if_exists systemctl "$@" --quiet disable || softfail --exit-status $? || return $?
}
  
app_units::disable_now() {
  app_units::run systemctl "$@" --quiet --now disable || softfail --exit-status $? || return $?
}

app_units::disable_now_if_exists() {
  app_units::run_if_exists systemctl "$@" --quiet --now disable || softfail --exit-status $? || return $?
}

app_units::start() {
  app_units::run systemctl "$@" --quiet start || softfail --exit-status $? || return $?
}

app_units::stop() {
  app_units::run systemctl "$@" stop || softfail --exit-status $? || return $?
}

app_units::stop_if_exists() {
  app_units::run_if_exists systemctl "$@" stop || softfail --exit-status $? || return $?
}

app_units::restart() {
  app_units::run systemctl "$@" restart || softfail --exit-status $? || return $?
}

app_units::restart_services() {
  app_units::run_for_services_only systemctl "$@" restart || softfail --exit-status $? || return $?
}
...
app_units::journal() {
  app_units::run_with_units journalctl "$@" || softfail --exit-status $? || return $?
}

app_units::follow_journal() {
  app_units::run_with_units journalctl --lines=1000 --follow "$@" || softfail --exit-status $? || return $?
}
