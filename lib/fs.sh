...
fs::mount-cifs() {
  ...
  if ! grep --quiet --fixed-strings --line-regexp "${fstabTag}" /etc/fstab; then
    ...
    echo "${serverPath} ${mountPoint} cifs credentials=${credentialsFile},file_mode=0640,dir_mode=0750,uid=${USER},gid=${USER},forceuid,forcegid,nosetuids,noposix,noserverino,echo_interval=10 0 0" | sudo tee --append /etc/fstab || fail
  fi
